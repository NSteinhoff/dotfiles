...
function s:gitlist_relative(cmd, filepath)
    return systemlist("git -C ".git#root_for_file(a:filepath)." ".a:cmd)
endfunction
...
function! git#original(filename, ref, n) abort
    ...
    return s:gitlist_relative("show ".a:ref."~".a:n.":".relpath, a:filename)
endfunction

" Get a the diff for a single file against a ref
function! git#patch(filename, ref)
    let relpath = git#relpath(a:filename)
    return s:gitlist_relative("diff ".a:ref." -- ".relpath, a:filename)
endfunction
