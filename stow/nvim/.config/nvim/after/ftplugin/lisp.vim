...
function LispOmnifunc(findstart, base)
    if a:findstart
        ...
        let matches = []
        for word in split(&lispwords, ',')
          if word =~ '^' . a:base
            call add(matches, { 'word': word, 'kind': 'lispword' })
          endif
        endfor
        for word in s:symbol_list
          if word =~ '^' . a:base
            call add(matches, { 'word': word, 'kind': 'symbol' })
          endif
        endfor
        return matches
      endif
...
setlocal omnifunc=LispOmnifunc
...
nmap <buffer> <silent> <BS> :'a,'bReplSend<CR>
